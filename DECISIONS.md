...
| **D24** | **One brand colour fails WCAG AA and is specified as secondary text.** `stone-500` #8C877F measures **3.39:1** on the warm white canvas; body text needs 4.5:1. `docs/DESIGN_SYSTEM.md` §7 states AA is required, not aspirational, so it and `docs/BRAND.md` disagree. | P0-06, P1-01 onward, P5-05 | **Narrowed by D16 on 2026-08-04 from four colours to one.** `teal` (9.12:1), `positive` (5.83:1) and `caution` (5.58:1) all pass comfortably on warm white — they only failed on the dark canvas, which is no longer being built. What remains is a single substitution: **#8C877F → #6F6A62** (5.10:1), the same hue moved darker until it is readable. In place and passing. The client accepts it or supplies their own replacement; the failing value cannot ship as body text either way. The three dark-canvas shades stay defined in `tokens.css` but are now unused. | 🔴 Open |
...
| **D24** | Fixing a colour that fails WCAG AA is a correction, not a redesign — and shipping the failing value is not an option either way. | Accept the substitution **provisionally** so work continues; show the client the before/after and let them supply their own. **Now a single colour** since D16 landed on light. |

**Unblocks 4 tasks** between them. D16 is answered as of 2026-08-04.
...
|---|---|---|---|
| **D16** | **Light — the warm white canvas.** The client's reason settles it better than either document did: *"My photos are mostly bright."* A dark canvas earns its keep by making low-light photography glow; against bright daytime images it fights them. He knows his own catalogue. Note this **overrides the evidence** found in `docs/BRAND.md` §Mood ("deep neutral canvas so that images do the talking") — that passage describes a mood, and the person who took the photographs outranks it. Implemented as one line in `lib/utils/canvas.ts`; all 92 E2E tests passed unchanged, which is what defining both token sets bought. | Client | 2026-08-04 |
...
| **D23** | **CSP keeps `script-src 'unsafe-inline'` for now, rather than going nonce-based.** Next.js can only apply a per-request nonce during dynamic rendering, so a strict CSP would disable static generation, ISR and CDN caching across the entire site — directly against `docs/TECHNICAL.md` §3 (statically generate the public shell) and §6 (LCP < 2.0s), and it would raise hosting cost. The vector `'unsafe-inline'` leaves open is injected inline script, and nothing in this app renders user-submitted HTML; content comes from a source we control and is Zod-validated. Every other directive is locked: `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, `frame-ancestors 'none'`, `connect-src 'self'`, `font-src 'self'`. The experimental `experimental.sri` option would give a strict policy *and* static rendering, but the security foundation should not rest on an experimental flag. **Revisit at P4-16** — the authenticated routes are dynamic anyway and can take a nonce policy for free. | Developer | 2026-08-03 |
| **D22** | **Locale detection is off (`localeDetection: false`).** `/` always lands on `/he`; the language is changed explicitly through the toggle (P1-13), never guessed from `Accept-Language`. Left on — next-intl's default — any browser reporting English is redirected to `/en`, which makes "Hebrew is the default locale" false in practice and, if D10 lands on "Hebrew only at launch", drops English-speaking visitors onto pages with no content. Caught by the P0-11 smoke test, not by reading the config. **Revisit once D10 is answered**: if English ships complete at launch, detection becomes defensible again. | Developer, from `docs/RULES.md` | 2026-08-03 |
| **D21** | **Scaffold toolchain (P0-02).** npm as the package manager — already present, natively supported by Vercel, and no case for adding an alternative. **TypeScript `^5` and ESLint `^9`, deliberately not the newer TypeScript 7 / ESLint 10** that npm now lists as latest: `eslint-config-next@16.2.12` targets ESLint 9, and this is the combination the Next.js team ships and tests. Also: `--empty` template (no demo boilerplate), no `src/` directory (`docs/TECHNICAL.md` §2 roots the tree at `/app`), `@/*` import alias, React Compiler off, Turbopack as the bundler (the Next 16 default). | Developer | 2026-08-03 |
